...
public abstract class BSynMAsynBizExecutor<T>{
	...
	final private void doExceute(String bizMethod,int totalSize,int page,int batchSize,Iterator<T> its){
		int t = 0;
		...
		LogUtils.timeused(_logger,"BSynMAsynBizExecutor.doExceute("+bizMethod+") ",start);
	}
	 ...
	 * @param object 处理对象
	 */
	abstract public void executes(T object);
